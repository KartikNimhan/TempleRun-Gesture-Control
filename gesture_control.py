...
# Gesture Thresholds
SWIPE_THRESHOLD = 80   # Horizontal movement (left/right)
JUMP_THRESHOLD = 60    # Hand moves up
DUCK_THRESHOLD = 40    # Hand moves down (made more sensitive)
HOLD_DURATION = 0.3    # Time to hold key

# Previous hand position (for gesture comparison)
prev_hand_x, prev_hand_y = None, None
last_action_time = 0
action_cooldown = 0.2  # Delay between actions
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if prev_hand_x is not None and prev_hand_y is not None:
                dx = wrist_x - prev_hand_x
                dy = index_y - prev_hand_y  # Using index finger for better vertical tracking
                ...
                # Detect duck (move down)
                elif dy > DUCK_THRESHOLD:
                    action = "DUCK"

            # Update previous positions
            prev_hand_x, prev_hand_y = wrist_x, index_y  # track wrist X and index Y
